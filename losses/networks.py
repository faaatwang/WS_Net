...
class InpaintGenerator(BaseNetwork):
    ...
    def Texture_Attention(self, x, y, mm):
        ...
        overlapping_weight = F.fold(F.unfold(torch.ones(mask.shape).type(x.dtype), kernel_size=(3, 3), padding=1),
                                    (h, w), kernel_size=(3, 3), padding=1)

        non_mask_region = torch.mean(mask_unfold, dim=1, keepdim=True).eq(
            0.).float()  ## Ensure the patch won't cover the mask pixel
...
class Discriminator(BaseNetwork):
    def __init__(
        ...
        conv1 = ([nn.ReflectionPad2d(1)] if use_reflection_pad else []) + [
            spectral_norm(nn.Conv2d(in_channels=in_channels, out_channels=64, kernel_size=4, stride=2, padding=conv_pad,
                                    bias=not use_spectral_norm), use_spectral_norm),
            nn.LeakyReLU(0.2, inplace=True),
        ]

        conv2 = ([nn.ReflectionPad2d(1)] if use_reflection_pad else []) + [
            spectral_norm(nn.Conv2d(in_channels=64, out_channels=128, kernel_size=4, stride=2, padding=conv_pad,
                                    bias=not use_spectral_norm), use_spectral_norm),
            nn.LeakyReLU(0.2, inplace=True),
        ]

        conv3 = ([nn.ReflectionPad2d(1)] if use_reflection_pad else []) + [
            spectral_norm(nn.Conv2d(in_channels=128, out_channels=256, kernel_size=4, stride=2, padding=conv_pad,
                                    bias=not use_spectral_norm), use_spectral_norm),
            nn.LeakyReLU(0.2, inplace=True),
        ]

        conv4 = ([nn.ReflectionPad2d(1)] if use_reflection_pad else []) + [
            spectral_norm(nn.Conv2d(in_channels=256, out_channels=512, kernel_size=4, stride=1, padding=conv_pad,
                                    bias=not use_spectral_norm), use_spectral_norm),
            nn.LeakyReLU(0.2, inplace=True),
        ]

        conv5 = ([nn.ReflectionPad2d(1)] if use_reflection_pad else []) + [
            spectral_norm(nn.Conv2d(in_channels=512, out_channels=1, kernel_size=4, stride=1, padding=conv_pad,
                                    bias=not use_spectral_norm), use_spectral_norm),
        ]
